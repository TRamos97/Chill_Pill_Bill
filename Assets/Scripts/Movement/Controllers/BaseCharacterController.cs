...
namespace ECM.Controllers
{
    ...
    public class BaseCharacterController : MonoBehaviour
    {
        ...
        protected bool _allowVerticalMovement;
        ...
        protected virtual void Jump()
        {
            ...
            if (isJumping)
            {
                ...
                {
                    _isJumping = false;
                    GetComponent<PlayerData>().PlayLandingSound();
                    playerData.PlayAnimation("JumpEnd", 0.75f);
                }
            }
            ...
            // proceed to jump if player is in climbing state
            
            if (!movement.isGrounded && _jumpUngroundedTimer > _jumpPostGroundedToleranceTime)
            {
                if(!playerData.climbableObject)
                    return;
            }
            ...
            if (playerData.climbableObject)
            {
                movement.cachedRigidbody.isKinematic = false;
                playerData.climbableObject.state = HookState.Relese;
                //playerData.climbableObject.inUse = false;
                //playerData.climbableObject = null;
            }
            playerData.PlayAnimation("JumpStart");
            playerData.PlayJumpSound();
        ...
        protected virtual void MidAirJump()
        {
            ...
            GetComponent<PlayerData>().ChangeShape();
            GetComponent<PlayerData>().ForwardPush();
